...
import {
  ...
  encodeAbiParameters, parseAbi, parseEther,
} from "viem"
...
const CHALLENGER_PK = keccak256(new TextEncoder().encode("dreamswipe-e2e-challenger-v1"))
...
const w0 = createWalletClient({ account: p0, chain: somniaTestnet, transport: http(RPC) })
const w1 = createWalletClient({ account: p1, chain: somniaTestnet, transport: http(RPC) })
...
if (bal1 < parseEther("0.02")) {
  const h = await w0.sendTransaction({ to: p1.address, value: parseEther("0.05") })
  await pub.waitForTransactionReceipt({ hash: h })
  step(0, `funded challenger ${p1.address.slice(0, 10)} with 0.05 STT`)
}

// 1 — commit a 3-card deck
const marketIds = [1, 2, 3].map((i) => keccak256(new TextEncoder().encode("mkt" + i)))
const strikes = [0n, 0n, 0n]
const salt = keccak256(new TextEncoder().encode("e2e-" + Date.now()))
const commit = keccak256(encodeAbiParameters(
...
const h1 = await w0.writeContract({ address: C, abi, functionName: "createDuel",
  args: [commit, 3, 0, "0x0000000000000000000000000000000000000000", 0n], gas: GAS })
...
// 3 — join
const h2 = await w1.writeContract({ address: C, abi, functionName: "joinDuel", args: [duelId], gas: GAS })
step(3, `joinDuel ${(await pub.waitForTransactionReceipt({ hash: h2 })).status}`)

// 4 — reveal (contract recomputes the commitment)
const h3 = await w0.writeContract({ address: C, abi, functionName: "revealDeck",
  args: [duelId, marketIds, strikes, salt], gas: GAS })
step(4, `revealDeck ${(await pub.waitForTransactionReceipt({ hash: h3 })).status}`)

// 5 — both players sign a swipe off-chain, relayer submits
const domain = { name: "DreamSwipeDuel", version: "1", chainId: 50312, verifyingContract: C } as const
const types = { Swipe: [
  { name: "duelId", type: "bytes32" }, { name: "cardIdx", type: "uint8" },
  { name: "direction", type: "uint8" }, { name: "nonce", type: "uint256" },
  { name: "deadline", type: "uint256" }] } as const
const deadline = BigInt(Math.floor(Date.now() / 1000) + 600)

for (const [w, acct, dir, label] of [[w0, p0, 0, "creator UP"], [w1, p1, 1, "challenger DOWN"]] as const) {
  const nonce = await pub.readContract({ address: C, abi, functionName: "nonces", args: [acct.address] }) as bigint
  const signature = await w.signTypedData({ account: acct, domain, types, primaryType: "Swipe",
  ...
  const h = await w0.writeContract({ address: C, abi, functionName: "recordSwipe",
    args: [duelId, acct.address, 0, dir, 400_000n, 1_000_000n, nonce, deadline, signature], gas: GAS })
...
for (let i = 0; i < 3; i++) {
  const h = await w0.writeContract({ address: C, abi, functionName: "settleCard",
    args: [duelId, i, 0, false], gas: GAS })
  await pub.waitForTransactionReceipt({ hash: h })
}
step(6, "settled 3/3 cards (card 0 → UP wins)")

// 7 — finalize and read the scores back
const h5 = await w0.writeContract({ address: C, abi, functionName: "finalize", args: [duelId], gas: GAS })
step(7, `finalize ${(await pub.waitForTransactionReceipt({ hash: h5 })).status}`)

const d = await pub.readContract({ address: C, abi, functionName: "getDuel", args: [duelId] }) as readonly unknown[]
const p0Score = d[12] as bigint, p1Score = d[13] as bigint
...
console.log(`   winner: ${p0Score > p1Score ? "creator" : p1Score > p0Score ? "challenger" : "draw"}`)
