...
const send = async (
  client: typeof w,
  params: Parameters<typeof w.writeContract>[0]
) => {
...
step(
  1,
  `mint  ${await send(w, { address: TOKEN, abi: erc20, functionName: "mint", args: [me.address, 100n * ONE], gas: GAS })}`
)
step(
  2,
  `approve ${await send(w, { address: TOKEN, abi: erc20, functionName: "approve", args: [POOL, 100n * ONE], gas: GAS })}`
)
step(
  3,
  `createSeason("${label}") ${await send(w, { address: POOL, abi: pool, functionName: "createSeason", args: [seasonId, TOKEN], gas: GAS })}`
)
step(
  4,
  `fund 100 ${await send(w, { address: POOL, abi: pool, functionName: "fund", args: [seasonId, 100n * ONE], gas: GAS })}`
)
step(
  5,
  `setAllocations(winner=60) ${await send(w, { address: POOL, abi: pool, functionName: "setAllocations", args: [seasonId, [winner.address], [60n * ONE]], gas: GAS })}`
)

// Before finalization the prize is NOT claimable — the UI must not offer it.
const preFinal = await pub.readContract({
  address: POOL,
  abi: pool,
  functionName: "claimable",
  args: [seasonId, winner.address],
})
step(
  6,
  `claimable before finalize = ${preFinal} (correctly 0 — allocations not frozen yet)`
)

step(
  7,
  `finalize ${await send(w, { address: POOL, abi: pool, functionName: "finalize", args: [seasonId], gas: GAS })}`
)
const postFinal = await pub.readContract({
  address: POOL,
  abi: pool,
  functionName: "claimable",
  args: [seasonId, winner.address],
...
step(
  9,
  `claim (sent BY the winner) ${await send(wWin, { address: POOL, abi: pool, functionName: "claim", args: [seasonId], gas: GAS })}`
)
...
try {
  ...
  console.log(
    `   double claim correctly rejected${blocked ? " (AlreadyClaimed)" : ""}`
  )
}
...
console.log(
  `   net paid: ${paid} — ${paid === 60n * ONE ? "exactly one allocation, no double pay" : "UNEXPECTED"}`
)
